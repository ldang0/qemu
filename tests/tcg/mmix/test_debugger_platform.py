...
def test_mmo_hosted_cold_reset(qemu, workdir):
    ...
    runtime_address = MMIX_STACK_SEGMENT_BASE + 0x400
    ...
    try:
        with _gdb_session(
            ...
            for iteration in range(2):
                ...
                remote.write_register(MMIX_GDB_RS, runtime_address)
...
def test_mmo_hosted_snapshot(qemu, workdir):
    ...
    data_address = MMIX_DATA_SEGMENT_BASE + 0x100
    stack_address = MMIX_STACK_SEGMENT_BASE + 0x600
    ...
    try:
        with _gdb_session(
            ...
            remote.write_register(MMIX_GDB_RS, stack_address)
            remote.write_register(MMIX_GDB_RL, 7)
            ...
            assert remote.read_register(MMIX_GDB_RS) == stack_address
